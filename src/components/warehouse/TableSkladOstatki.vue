...
<div class="col" style="width: 100%;">
    ...
    <InfoOstatkiModal v-model:dialog="dialogInfoOstatkiModal" :row="selectedRow" />
</div>
</template>
...
import InfoOstatkiModal from 'src/components/warehouse/warehousemodal/InfoOstatkiModal.vue';
...
{ name: 'costPriceTotal', required: true, align: 'right', label: 'Себестоимость (итого)', field: row => row.costPriceTotal, format: val => `${val}`, sortable: true },
...
const dialogInfoOstatkiModal = ref(false);
const selectedRow = ref({});

const openWindow = (row) => {
selectedRow.value = row;
dialogInfoOstatkiModal.value = true;
};
